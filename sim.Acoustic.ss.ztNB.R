...
sim.Acoustic.ss.ztNB <-
  function(N=NA,beta0=NA,beta1=NA,sigma.d=NA,sigma.u=NA,mindB=NA,X=NA,buff=NA,lambda.C=NA,theta.C=NA){
    # simulate a population of activity centers
    xlim <- c(min(X[,1])-buff,max(X[,1])+buff)
    ylim <- c(min(X[,2])-buff,max(X[,2])+buff)
    s <- cbind(runif(N, xlim[1],xlim[2]), runif(N,ylim[1],ylim[2]))
    J <- nrow(X)
    # simulate number of calls.
    n.calls <- rnbinom(N,mu=lambda.C,size=theta.C)
    for(i in 1:N){#ensure no counts of 0
      if(n.calls[i]==0){
        idx <- 1
        while(n.calls[i]==0){
          n.calls[i] <- rnbinom(1,mu=lambda.C,size=theta.C)
          idx <- idx+1
          if(idx==1000)stop("stuck in infinite loop trying to simulate ztNB counts that are not 0. Are these reasonable parameters?")
        }
      }
    }
